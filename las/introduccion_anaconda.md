...
lidar    ← entorno de este curso
otro     ← otro entorno independiente
```
...
# Entorno desde un fichero environment.yml
...
- Por eso, si usas un `environment.yml`, el orden importa: primero todo lo que venga de conda, y al final el bloque `pip:` solo para lo que no esté en ningún canal conda.
...
conda install -c conda-forge pdal python-pdal proj-data laspy lazrs-python \
              rasterio geopandas shapely matplotlib numpy jupyterlab
```

> **`proj-data`** es imprescindible: instala el geoide oficial español
> `es_ign_egm08-rednap.tif`, necesario para convertir alturas elipsoidales
> (WGS84, lo que exporta DJI Terra) a alturas ortométricas (lo que usa el PNOA).

### Verificar el geoide

Una vez instalado el entorno, verifica que PROJ encuentra el geoide oficial:

```bash
projinfo -s EPSG:4979 -t EPSG:25830+5782
```

En la Operación nº 1 del resultado debe aparecer:

```
+step +inv +proj=vgridshift +grids=es_ign_egm08-rednap.tif +multiplier=1
```

Si aparece `at least one grid missing` en todas las operaciones, reinstala `proj-data`:

```bash
conda install -c conda-forge proj-data --force-reinstall
```

### Alternativa: instalar desde `environment.yml`

Si tienes el fichero `environment.yml` del repositorio:
...
El entorno de este curso tiene la siguiente estructura:
...
Cuando lanzas JupyterLab, los notebooks necesitan un **kernel** — el proceso Python que ejecuta el código. Por defecto, Jupyter usa el kernel del entorno desde el que se lanza. Para que tus entornos conda aparezcan como kernels disponibles hay dos formas:
...
Esto registra ese entorno concreto como kernel disponible para el usuario.
...
> **Importante:** el kernel seleccionado en el notebook (esquina superior derecha en JupyterLab)
> es lo que determina qué librerías están disponibles. Asegúrate siempre de tener seleccionado
> el kernel **`lidar`** antes de ejecutar las celdas.
...
Se abrirá el navegador automáticamente en `http://localhost:8888`. Si no, copia la URL que aparece en la terminal.
...
| Acción | Comando |
|--------|---------|
| Crear entorno del curso | `conda create -n lidar python=3.11` |
| Crear entorno desde fichero | `conda env create -f environment.yml` |
| Activar entorno | `conda activate lidar` |
